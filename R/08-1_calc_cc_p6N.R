## Configuration
analysis_name <- "09_1.Create_CC_6N_N15"
...
for(chr in paste0("chr", 1:22)){
  df_6N <- read_(paste0("df_cov10_6N_", chr), "create_CC_input_cov")
  colnames(df_6N) = gsub(".6N.", "_", gsub("hg38.", "", colnames(df_6N)))
  
  ### Test new algorithm
  batch_n = 10000
  batch = round(nrow(df_6N) / batch_n) - 1
  
  for(b in 0:batch){
    start_idx = ifelse(b==0, 1, b*10000+1)
    end_idx = ifelse(b==batch, nrow(df_6N), (b+1)*10000)
    print(paste0(b, " ", start_idx, " ", end_idx, " ", Sys.time()))
    df_batch = df_6N[start_idx:end_idx,]
    ...
    save_(paste0("df_wg_cc_", chr,"_6N_N15_batch", b), data=results_df)
  }
  
  #merge
  df = data.frame()
  for(b in 0:batch){
    df_t = read_(paste0("df_wg_cc_", chr,"_6N_N15_batch", b), "09_1.Create_CC_6N_N15")
    df = rbind(df, df_t)
  }
  
  save_(paste0("df_merged_6N_N15_", chr), data=df) 
}
