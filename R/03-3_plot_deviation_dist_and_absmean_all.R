## Configuration
analysis_name <- "03-2_plot_deviation_dist_and_absmean_all"
...
##
selected_loci = c("m10", "m8", "m3", "m4", "m6", "m12")
beta_cov_gs_ds_tb %>% filter(lid %in% selected_loci)
#beta_cov_gs_ds_tb = beta_cov_gs_ds_tb %>% filter(workflow %in% c("bwa-meth", "methylCtools", "GSNAP", "Biscuit", "Bismark", "BSBolt"))
...
plot_gs_abs_metric_condensed <- function(var, title=NA, mean_lab="mean abs", median_lab="median abs"){
  ...
  mean_plot = mean_plot + ylim(c(0, 0.105)) + 
