analysis_name <- "07-5_plot_bismark_beta_dist_ds"
config = file.path(getwd(), "0_project_setting.R")
source(config)

methrix_obj_dir = "/omics/groups/OE0219/internal/yuyu/benchmarking/analysis/read_in_hdf5_ds"

## Load
##########################
meth_list = lapply(c("BSBolt", "Biscuit", "Bismark", "GSNAP", "bwa-meth", "methylCtools"), function(x){
...
save_("df_tmp", data=df_tmp)

df_tmp %>% filter(protocol=="WGBS")
total = nrow(df_tmp)

p<-ggplot(df_tmp, aes(x=beta)) + 
  geom_histogram(binwidth = 0.01, fill="#CD534CFF", color="darkred") +  unified_pg + 
...
p<-ggplot(df_tmp, aes(x=beta)) + 
  geom_histogram(binwidth = 0.01, fill="#CD534CFF", color="#CD534CFF") +  unified_pg + 
  theme(panel.background = element_rect(fill = "white", colour = "black", linetype="solid"),
        panel.border=element_rect(fill=NA),
        strip.background = element_blank(),
        legend.position='none') + 
...
p_type <- df_tmp %>% dplyr::filter(sample %in% c("5N", "6N")) %>% ggplot(aes(x=beta, color=protocol)) +
    geom_histogram(binwidth = 0.01)+ unified_pg +
    facet_wrap(~protocol, ncol=5) + color_palette_color() + color_palette_fill() + ylab(paste0("5N and 6N")) +
    theme(panel.background = element_rect(fill = "white", colour = "black", linetype="solid"),
          panel.border=element_rect(fill=NA),
          strip.background = element_blank(),
          legend.position='none')
